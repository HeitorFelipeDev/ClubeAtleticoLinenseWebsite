...
function getFormDados() {
  let dados = {
    nomePatrocinador: document.querySelector("#input-nome-patrocinador").value,
    linkPatrocinador: document.querySelector("#input-link-patrocinador").value,
  ...
  dataForm.append("nome_patrocinador", dados.nomePatrocinador);
  dataForm.append("link", dados.linkPatrocinador);
  dataForm.append("id_imagem", dados.idImagemSelecionada);

  let url = `http://localhost/WEBSITELINENSE/api/model/patrocinadorController.php?operation=create`;
...
async function requestImages() {
  ...
  if(data.length > 0) {
    let images = data.filter((item) => item.categoria === "patrocinador");
    showImages(images);
  } else {
    showImages([]);
  }
  ...
  if(main.length == 0) {
    ...
    imagesSelected.forEach((imagem) => {
      imagem.addEventListener("click", () => {
        ...
        var buttonCancelCreatePatrocinador = document.querySelector(".button-cancelar");
        buttonCancelCreatePatrocinador.addEventListener("click", () => {
          imagem.parentElement.classList.remove("selected");
        });
  
        var buttonVoltarCreatePatrocinador = document.querySelector(
          ".button-voltar-icon"
        );
        buttonVoltarCreatePatrocinador.addEventListener("click", () => {
          imagem.parentElement.classList.remove("selected");
        });
...
function requestReadData() {
    fetch(
      "http://localhost/WEBSITELINENSE/api/model/patrocinadorController.php?operation=read"
    )
...
function preencherTabelaComJSON() {
  ...
  var tituloFiltrado = document
    .getElementById("input-search-patrocinadores")
    .value.trim()
    .toLowerCase();
  if (tituloFiltrado !== "") {
    sortedData = sortedData.filter(function (item) {
      return item.nome_patrocinador.toLowerCase().includes(tituloFiltrado);
    });
  ...
  sortedData.sort(
    (a, b) =>
      ...
      (new Date(a.id_patrocinador) - new Date(b.id_patrocinador))
  );
  ...
  for (var i = start; i < end && i < sortedData.length; i++) {
    ...
    linha.setAttribute("data-id", sortedData[i].id_patrocinador);

    var editarIcon = document.createElement("i");
    editarIcon.className = "fas fa-edit edit-icon";
    editarIcon.setAttribute("title", "Editar Patrocinador");

    editarIcon.addEventListener("click", function (event) {
      // EDIT-PATROCINADOR
      let formEditContainer = document.querySelector(
        ".form-edit-patrocinador-container"
      );
      document.querySelector(".list-patrocinadores-container").style.display = "none";
      formEditContainer.style.display = "block";

      formEditContainer
        .querySelector(".button-voltar-icon")
        .addEventListener("click", () => {
          document.querySelector(".list-patrocinadores-container").style.display =
            "block";
      ...
      // set values
      formEditContainer.querySelector("#input-nome-patrocinador").value =
        linhaSelecionada.nome_patrocinador;
      formEditContainer.querySelector("#input-link-patrocinador").value =
      ...
      let buttonUploadPatrocinador = document.querySelector("#button-upload-patrocinador");
      buttonUploadPatrocinador.addEventListener("click", uploadPatrocinador);

      function uploadPatrocinador() {
        let dados = {
          idPatrocinador: linhaSelecionada.id_patrocinador,
          nomePatrocinador: formEditContainer.querySelector("#input-nome-patrocinador").value,
          linkPatrocinador: formEditContainer.querySelector("#input-link-patrocinador").value,
          idImagem: idImagem,
        };
        ...
        let dataForm = new FormData();
        dataForm.append("id_patrocinador", dados.idPatrocinador);
        dataForm.append("nome_patrocinador", dados.nomePatrocinador);
        dataForm.append("link", dados.linkPatrocinador);
        dataForm.append("id_imagem", dados.idImagem);

        let url = `http://localhost/WEBSITELINENSE/api/model/patrocinadorController.php?operation=update`;
      
        requestUpdatePatrocinador(url, dataForm);
        console.log(dados);
      }
    ...
    apagarIcon.setAttribute("title", "Deletar Patrocinador");
    apagarIcon.addEventListener("click", function (event) {
      //  DELETE PATROCINADOR
      Swal.fire({
        title: "Deletar Patrocinador",
        text: "Tem certeza de que deseja deletar esse patrocinador? Uma vez que o mesmo for deletado não poderá ser recuperado!",
        icon: "warning",
    ...
    viewIcon.addEventListener("click", function (event) {
      // VIEW DETAILS OF THE PATROCINADOR
      let containerViewImageModal = document.querySelector(
        ".container-view-patrocinador"
      );
      ...
      function setDataModal() {
        containerViewImageModal.querySelector("#id-patrocinador").innerText =
          "000" + linhaSelecionada.id_patrocinador;
        containerViewImageModal.querySelector("#nome-patrocinador").innerText =
          linhaSelecionada.nome_patrocinador;
        containerViewImageModal.querySelector("#link-patrocinador").innerHTML = linhaSelecionada.link;
 
        containerViewImageModal.querySelector("#id-imagem-patrocinador").innerText =
  ...
  function showImage(_src) {
    const patrocinadorImage = 
    containerViewImageModal.querySelector("#patrocinador-view-img");

    if (_src && Array.isArray(_src) && _src.length > 0 && _src[0].path) {
      patrocinadorImage.src = _src[0].path.replace("../../", "");
    } else {
      // Caso o patrocinador não tenha uma imagem associada, defina a imagem padrão.
      patrocinadorImage.src = "http://localhost/WEBSITELINENSE/assets/images/image-default.jpg";
    }
  }
}
    });
    ...
    idCell.textContent = sortedData[i].id_patrocinador;
    nomeCell.textContent = sortedData[i].nome_patrocinador;
...
document
  .getElementById("button-submit-search-patrocinadores")
  .addEventListener("click", applyTitleFilter);

var buttonAddNewPatrocinador = document.querySelector("#button-create-new-patrocinador");
buttonAddNewPatrocinador.addEventListener("click", showCreateNewPatrocinadorContainer);

function showCreateNewPatrocinadorContainer() {
  let list = document.querySelector(".list-patrocinadores-container");
  list.style.display = "none";
  let container = document.querySelector(".form-create-patrocinador-container");
...
// REQUESTS
async function requestDeletePatrocinador(id) {
  let url = `http://localhost/WEBSITELINENSE/api/model/patrocinadorController.php?operation=delete&id_patrocinador=${id}`;
  let req = await fetch(url);
...
async function requestUpdatePatrocinador(_url, _dataForm) {
...
document
  .querySelector("#input-search-patrocinadores")
...
document.querySelector("#input-search-patrocinadores").addEventListener("keyup", () => {
  applyTitleFilter();
});
